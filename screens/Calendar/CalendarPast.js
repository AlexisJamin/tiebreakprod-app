  ...
  componentDidMount() {
   ...
   /* .then(()=> {
    ...
    .then(()=> {
        ...
        var data = _.orderBy(dataMerge , ['date'], ['desc']);
  ...
  renderEmpty() {
    ...
    return (
      ...
      >
        ...
        <Text style={{marginTop:20}}> {translate.noGame[this.props.user.currentLocale]}.</Text>
      </View>
    );
  }
   ...
   >
      <SectionList
        ...
        renderItem={({ item }) => (
          ...
          title={
            <Text style={{fontSize:12, paddingTop:2}}>{translate.club[this.props.user.currentLocale]}: {item.club.name || item.clubName}</Text>
          }
          subtitle={
            <View>
            { item.typeOf === "game" &&
...
export default connect(mapStateToProps, mapDispatchToProps) (CalendarPast);
